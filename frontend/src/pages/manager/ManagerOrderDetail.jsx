...
import { getStatusBadgeClass, formatStatusLabel, getNextStatuses } from '../../utils/orderStatus';
...
export default function ManagerOrderDetail() {
  ...
  async function updateStatus(nextStatus) {
    if (nextStatus === 'rejected' && !confirm('Reject this order?')) return;
    setUpdating(nextStatus);
    setActionError('');
    try {
      const response = await client.put(`/manager/orders/${id}/status`, { status: nextStatus });
      setOrder(response.data.order);
    } catch (err) {
        ...
        <div className="action-row">
          {nextStatuses.map((nextStatus) => (
            <button
              key={nextStatus}
              type="button"
              className={nextStatus === 'rejected' ? 'btn btn-danger' : 'btn btn-primary'}
